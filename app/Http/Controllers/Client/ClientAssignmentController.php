...
class ClientAssignmentController extends Controller
{
    ...
    public function index()
    {
        ...
        $assignments = ProgramAssignment::with(['program.trainer.user'])
            ...
            ->map(function ($assignment) {
                ...
                $assignment->can_delete = $assignment->canBeDeleted();
                return $assignment;
            });
    ...
    /**
     * Cancel payment and withdraw from program (for pending_payment status)
     */
    public function cancelPaymentAndWithdraw(Request $request, $id)
    {
        $request->validate([
            'reason' => 'nullable|string|max:500',
        ]);
        ...
        $assignment = ProgramAssignment::with(['program', 'program.trainer.user', 'payments'])
            ->where('client_id', $client->id)
            ->findOrFail($id);

        // Check if cancellation is allowed (must be pending_payment)
        if (!$assignment->isPendingPayment()) {
            return redirect()->back()
                ->with('error', 'This program is not in a state where payment can be cancelled.');
        }

        DB::transaction(function () use ($assignment, $request, $client) {
            // Update assignment status to withdrawn
            $assignment->update([
                'status' => ProgramAssignmentStatus::WITHDRAWN,
                'notes' => $request->reason ? 'Payment cancelled by client: ' . $request->reason : 'Client cancelled payment and withdrew from program',
                'payment_deadline' => null, // Clear payment deadline
            ]);

            // Handle any existing payments (mark as cancelled if pending)
            ...
            // Check if client has any remaining assignments with this trainer
            $remainingAssignmentsWithTrainer = ProgramAssignment::where('client_id', $client->id)
            ...
            // Update trainer relationship if needed
            if ($remainingAssignmentsWithTrainer === 0) {
                $newTrainerCount = max(0, $client->trainer_count - 1);
                if ($newTrainerCount === 0) {
                    $client->update([
            ...
            $notification = \App\Models\Notification::create([
                ...
                'title' => 'Client Cancelled Payment',
                'message' => "Client {$client->user->full_name} has cancelled payment and withdrawn from your program '{$assignment->program->name}'." .
                           ($request->reason ? " Reason: {$request->reason}" : ""),
                'data' => [
                    'program_id' => $assignment->program->id,
                    'client_id' => $client->id,
                    ...
                    'amount' => $assignment->program->price,
                ],
            ]);
            ...
            Log::info('Client cancelled payment and withdrew', [
                'client_id' => $client->id,
                'assignment_id' => $assignment->id,
                'program_id' => $assignment->program->id,
        ...
        return redirect()->route('client.assignments.index')
            ->with('success', 'Payment cancelled and you have withdrawn from the program.');
    }

    /**
     * Delete a program assignment (only for withdrawn/rejected/cancelled programs)
     */
    public function destroy($id)
    {
        ...
        if (!$assignment->canBeDeleted()) {
            return redirect()->back()
                ->with('error', 'This program cannot be deleted. You can only delete withdrawn, rejected, or cancelled programs.');
        }
        ...
        Log::info('Client deleted program assignment', [
            'client_id' => $client->id,
            'assignment_id' => $id,
        ...
        return redirect()->route('client.assignments.index')
            ->with('success', "Program '{$programName}' has been removed from your list.");
    }
